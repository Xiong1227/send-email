...
不必把程序长期挂在自己电脑上，也不必租一台云服务器。仓库已接好 **GitHub Actions**：推到 GitHub、填好密钥后，默认每天北京时间 **08:00** 和 **18:00** 各跑一次，电脑关机简报也会到。这是本项目面向日常使用的推荐方式。
...
- 从多个 RSS 源拉取最新报道，抓取正文后交给 LLM 做分层摘要
- 邮件按「30秒速览 → 深度精选 → 快讯」组织，标题可点进原文；并标明每条新闻是几小时/几天前
- 生成带篇幅提示的 HTML 邮件（约 xx 字、大约阅读时间）；新鲜资讯过少时主题加「低活跃」
...
4. 成功后即可依赖定时：北京时间每天 08:00、18:00（工作流里写的是 UTC `00:00` / `10:00`）。也可随时再点 Run 补发一次。
...
3. **LLM 整理**：按「30秒速览 → 关键数据 → 深度精选 → 快讯」分层输出。标题做成可点链接；每条带「3小时前 / 2天前」时效。脚本再补上北京时间、篇幅和新鲜度统计。
...
| `briefing.fresh_hours` | 视为「新鲜」的小时数，默认 12 |
| `briefing.min_fresh_articles` | 少于此数则主题加「低活跃」 |
| `briefing.weight_keywords` | 用于选出速览/精选的关键词，可按兴趣改 |
...
编辑 `config.yaml` 的 `rss_feeds`，换成你关心的站点。数量和领域都可以自己定；`max_articles` 控制每个源的条数，避免一次塞给模型太多内容。`briefing.weight_keywords` 可改成你关心的词（如房价、芯片），命中的标题更容易进「30秒速览」。
